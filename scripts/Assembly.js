class Assembly { // for now, all assemblies are assumed to be centered on origin (X and Z directions). lowest point will be at Y = 0
    ...
    static parse(assemblyCfg, materialsCfg) { // parses the Assembly cfg and returns an assembly (loads all models from assembly into scene)
        ...
        Object.keys(loadModels).forEach(modelKey => {
            ...
            Assembly.loadSTL(loader, srcPath, modelCfg, modelKey, meta.materials, models, () => loaded += 1)
            // TODO: add to toggle bar
        })

        const togglesCfg = assemblyCfg.toggles
        Object.keys(togglesCfg).forEach(toggleCfgKey => {
          toolbar.addModelsToggle(toggleCfgKey, togglesCfg[toggleCfgKey])
        });
    ...
    static loadGLTF(loader, srcPath, modelCfg, modelKey, materials, modelDict, callback) { // loads a gltf model with given params, adding itself into the modelDict when loaded
    ...
    static loadSTL(loader, srcPath, modelCfg, modelKey, materials, modelDict, callback) { // loads a gltf model with given params, adding itself into the modelDict when loaded
